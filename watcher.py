...
async def check_account(name: str, session_path: str):
    ...
    try:
        ...
        async for dialog in client.get_dialogs():
            if dialog.unread_messages_count > 0 and dialog.chat.type.value not in ("channel", "supergroup", "group"):
...
async def run_session():
    ...
    await send_notification(f"🟢 Starting session — accounts: {len(sessions)}")
    ...
    await send_notification("✅ Session completed")